...
## Amendment, 2026-09: the calendar is the venue's, not only the reader's

§5 said the calendar follows the reader, along with the digits and the decimal separator, and gave
the sharp case for it: a Persian reader handed ۲۹ سپتامبر ۲۰۲۶ has to convert it before it names a
day they could turn up on. That reasoning is still right and it is incomplete, because it answers
only half of the question.

**The language is a fact about the reader. The calendar is a decision by the organisation.** An
Iranian theatre whose page is in English still programmes its season in Jalali and prints ۱۴۰۵ on the
ticket; a venue in Dubai reading Persian may keep its books in Gregorian and want the door list to
match. Deriving one from the other made both of those impossible to express.

So the calendar is now a setting with three values — follow the reader's language, Jalali, or
Gregorian — and `auto` is §5's rule kept as a deliberate default rather than as the only option.
There are two of them, because they answer two questions: a **site's** is what a buyer reads, and an
**account's** is what staff read and type. They are usually the same and they are not always.

Three things this cost, each worth writing down:

- **One resolution point, or none.** `App\Support\Locale\Calendars` holds the choice for the request;
  `Dates` formats with it and `Locales::icu()` puts it into the locale string handed to the browser,
  so the panel's own JavaScript follows without knowing the class exists. Binding an account binds
  its calendar, which is what makes a report generated by a queue worker come out right.
- **A public cache must not carry a private choice.** `/v1/i18n/{locale}` is cached for a day and
  marked `public`; its `icu` is deliberately the *language's* own calendar, because anything
  account-shaped in it would reach the next account through a shared cache. The venue's own arrives
  with the sign-in and the two are composed in the browser.
- **Showing is the easy half; typing is the feature.** A browser's date control is Gregorian and
  cannot be asked otherwise, so in Jalali every date input in the panel is upgraded in place — the
  original keeps its name and its value, and a box beside it reads and writes Jalali. In Gregorian
  nothing is touched, because the native control knows the reader's keyboard and screen reader and
  is better than anything we would write.

The conversion is Borkowski's algorithm rather than a dependency, and it is checked against ICU for
every day between 1990 and 2060 in both directions. That check earned its place immediately: it
caught an integer division that floored where the algorithm truncates — wrong by a day in one year
and by two years in another — and a leap-year flag read from the wrong year. Neither would have been
visible in a spot check, and both would have put the wrong date on a poster.
