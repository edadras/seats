...
The calendar is the one of those that is *not* only about the reader, and it has its own setting —
see **Two calendars, and the venue says which**. A venue that programmes its season in Jalali prints
Jalali on an English page too.
...
## Two calendars, and the venue says which

Dates used to follow the reader's language: Persian got the Persian calendar, everyone else
Gregorian. That is a sensible default and it answers the wrong question. The language is a fact about
the **reader**; the calendar is a decision by the **organisation**. An Iranian theatre whose page is
in English still programmes its season in Jalali and prints ۱۴۰۵ on the ticket, and a venue in Dubai
reading Persian may keep its books in Gregorian and want the door list to match.

So there are two settings, and they are two different questions rather than one duplicated:

- **The site's**, on its Languages screen — what a *buyer* reads, on the programme, in the basket and
  on the ticket.
- **The account's**, beside the language picker in the panel's own footer — what *staff* read and
  type, in the panel, the door list and the reports.

Both offer the same three: follow the reader's language (the old behaviour, and still the default),
Jalali, or Gregorian. They are usually the same and they are not always: a venue selling to visitors
from abroad may publish Gregorian dates and still run its box office in Jalali.

**One place decides and everything follows.** `App\Support\Locale\Calendars` holds the choice for
the request; `Dates` formats with it and `Locales::icu()` puts it into the locale string handed to
the browser — so the panel's own JavaScript changes with it without knowing the class exists.
Binding an account binds its calendar, which is what makes a report generated by a queue worker at
four in the morning come out right.

**Nothing stored moves.** Everything is UTC in the database and a wall clock in the venue's own zone
on the way out. A calendar is a way of writing a moment down; it is not a different moment.

**The half that decides whether this is real is typing, not showing.** A browser's own date control
is Gregorian and cannot be asked otherwise, so in Jalali every date box in the panel is upgraded in
place: the original input stays, keeping its name and its value in the format the server expects,
and a box beside it shows and accepts the same instant in Jalali, with a month to pick a day out of.
That is why it works on twenty-five date fields across fourteen screens without any of them being
edited, and on the ones written next year too. In Gregorian nothing happens at all — the native
control knows the reader's keyboard and their screen reader, and is better than anything here.

The conversion is Borkowski's algorithm rather than a library, and `calendar_smoke` checks it against
ICU's own answer for **every day between 1990 and 2060, in both directions**. That found two real
bugs while it was being written: an integer division that floored where the algorithm wants
truncation — wrong by a day in one year and by two years in another — and a leap-year flag read from
the year after the one it belongs to.
...
| A venue that keeps Jalali prints Jalali on an English page | `CalendarTest`, `calendar_smoke` |
| A venue that keeps Gregorian prints it to a Persian reader | `CalendarTest` |
| The same moment written two ways is one moment | `CalendarTest` |
| A site and an account decide separately, and a clerk decides neither | `CalendarTest` |
| The cached language catalogue carries no venue's choice | `CalendarTest` |
